...
async def checkout(request: CheckoutRequest) -> dict:
    async with httpx.AsyncClient(timeout=5.0) as client:
        order_response = await client.post(f"{ORDER_SERVICE}/orders", json=request.model_dump())
        order_response.raise_for_status()
        recs_response = await client.get(f"{RECOMMENDATION_SERVICE}/recommendations/{request.user_id}")
